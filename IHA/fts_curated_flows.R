fts_curated_flows <- function(years = 2016:2022, update_years = 2022:2022){
  ...
  fts_files <- list.files(path = "IHA/datasets", pattern = "fts_")
  fts_list <- list()
  for(i in 1:length(years)){
    ...
    if(!(paste0("fts_", years[i], ".csv") %in% fts_files) | years[i] %in% update_years){
      ...
      fwrite(fts, paste0("IHA/datasets/fts_", years[i], ".csv"))
    }
    message(paste0("Reading ", years[i]))
    fts_list[[i]] <- fread(paste0("IHA/datasets/fts_", years[i], ".csv"), encoding = "UTF-8")
  }
  ...
  #Merge orgs types
  fts <- merge(fts, fts_orgs, by = "sourceObjects_Organization.id", all.x = T)
  fts[org_type != "Government" | is.na(donor_country), donor_country := "Total DAC"]
  
  #Deflate
  deflators <- fread("reference_datasets/usd_deflators_2021WEO.csv", header = T)
