...
#Aggregate
agg_channels <- fts_curated[, .(total_2020USD = sum(amountUSD_defl, na.rm = T)), by = .(year, gha_channel, channel, subchannel, status)][order(year, gha_channel, channel, subchannel, status)]
